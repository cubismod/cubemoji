...
// logic for actually performing image manipulation
// in Discord
namespace DiscordLogic {
  const gmManager = container.resolve(GmManager)
  ...
  export async function rescale (context: MsgContext, source: string, user: User | PartialUser) {
    ...
    if (url) {
      startTyping(context)
      // add rescale job to the queue
      const filename = await gmManager.enqueueJob({
        inPath: source,
